...
docker build -t maccommerce/inventory-service:latest -t maccommerce/inventory-service:"$PROJECT_VERSION" .

echo "${BLUE_COLOR}${BOLD}Pushing docker image to Docker Hub...${NO_COLOR}${NORMAL}"
docker push maccommerce/inventory-service:latest
docker push maccommerce/inventory-service:"$PROJECT_VERSION"
